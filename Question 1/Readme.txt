...
-> Each player who has arrived and met the organiser busily waits to enter court by calling enterCourt() where a while loop is running
	until the state of the corresponding player is changed by the assign().
...
-> Several mutexes used to ensure only one person is able to enter court at a time, only one person is able to enter academy at a
	time.    